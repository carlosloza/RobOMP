...
% in the RobOMP package DOI: 10.7717/peerj-cs.192 (open access)
...
% Note: Several noise variances and number of iterations are allowed
...
sigma_v = 0:0.5:5;          % Set of variances of added zero-mean gaussian noise       
...
for i = 1:nSigma
    ...
    for it = 1:n_it
        ...
        tic
        xCorrOMP = CMP(y, D, 'nnonzero', K);
        time_CMP(i, it) = toc;
        err_CMP(i, it) = norm(x0 - xCorrOMP)/norm(x0);
