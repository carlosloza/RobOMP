...
% Missing pixels noise vs feature dimension scenario
% Author: Carlos Loza
...
% - This script replicates the results summarized in Fig 8A of RobOMP
% article
...
% computationally demanding, e.g. it took roughly 1.66 days to run the 10
...
misspixrate = 0.7;          % Missing pixels rate
...
for down_i = 1:ndown
    ...
    for rep = 1:nRep
        clc
        fprintf('Feature dimension = %d, Missing pixels rate = %.2f, Repetition: %d \n', size(Y,1), misspixrate, rep)
        ...
        for sub_i = 1:nSub
            ...
            YNoise = zeros(size(Y));
            for i = 1:size(Y, 2)
                idxMiss = randperm(round(m*n), round(misspixrate*m*n));
                auxVecNoise = Y(:,i);
                % Adding noise here
                auxVecNoise(idxMiss) = max(auxVecNoise)*rand(round(misspixrate*m*n),1);
